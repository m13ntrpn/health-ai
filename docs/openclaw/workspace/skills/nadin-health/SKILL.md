...
Скрипт автоматически определяет тип вызова:
- **query-процедуры** (получение данных) — GET-запрос с input в URL.
- **mutation-процедуры** (запись данных) — POST-запрос с телом.

Ты ничего не настраиваешь вручную — просто передай имя процедуры и JSON-объект.
...
**В nadin.sh передавай путь процедуры `user.isProfileComplete`, не имя checkProfile.**
