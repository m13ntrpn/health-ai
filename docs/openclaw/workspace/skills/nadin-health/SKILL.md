...
Всегда вызывай через одну форму:
`bash {baseDir}/nadin.sh <procedure> '<json_payload>'`.

- `procedure` — путь процедуры (см. ниже).
- `json_payload` — объект с `telegramUserId` и нужными полями.
...
Первый аргумент `nadin.sh` — **путь процедуры** (например `user.isProfileComplete`). Используй путь из примеров ниже, не короткие имена вроде checkProfile или saveProfile — иначе API вернёт 404.

### 1. checkProfile — проверить анкету (`user.isProfileComplete`)
...
### 2. saveProfile — сохранить анкету (`user.upsertProfile`)
```
...
```
Все поля payload опциональны — передавай только то, что известно.

---

### 3. saveDailyLog — записать данные за день (`healthLog.upsertDailyLogForTelegramUser`)
```
...
### 4. getDailyLog — лог за день (`healthLog.getDailyLogForTelegramUser`)
```
...
### 5. listDailyLogs — список логов (`healthLog.listDailyLogsForTelegramUser`)
```
...
### 6. getSummary — числовая сводка за период (`healthLog.summaryForTelegramUser`)
```
...
### 7. getReport — полный контекст для отчёта (`healthLog.reportForTelegramUser`)
```
...
### 8. addBodyMeasurement — замер тела (`bodyMeasurement.addForTelegramUser`)
```
...
```
Все поля, кроме `telegramUserId`, опциональны.

---

### 9. listBodyMeasurements — список замеров тела (`bodyMeasurement.listForTelegramUser`)
```
...
### 10. addMedicationPlan — постоянный препарат (курс) (`medicationPlan.addForTelegramUser`)
```
...
### 11. listMedicationPlans — список постоянных препаратов (`medicationPlan.listForTelegramUser`)
```
...
### 12. addLabResult — анализы в свободной форме (`labResult.addForTelegramUser`)
```
...
### 13. addLabPanel — структурированные анализы (`labPanel.addForTelegramUser`)
```
