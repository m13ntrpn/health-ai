...
**Важно (только для тебя, не для пользователя):**
- Никогда не цитируй команды bash, `curl` или JSON из них в ответе пользователю.
- Для всех полей дат (`date`, `fromDate`, `toDate`) всегда передавай непустую строку в формате `YYYY-MM-DD`. Не используй значения `today`, `null`, `undefined` или пустую строку.
- **Для простых payload (мало полей)** — передавай JSON вторым аргументом в одинарных кавычках.
- **Для больших payload (много блюд, сон + еда + вода одновременно)** — всегда используй heredoc через stdin, чтобы избежать проблем экранирования:
  ```bash
  cat <<'PAYLOAD_EOF' | bash {baseDir}/nadin.sh healthLog.upsertDailyLogForTelegramUser
  {"telegramUserId":"<ID>","date":"YYYY-MM-DD","payload":{...}}
  PAYLOAD_EOF
  ```
- Скрипт сам проверит корректность JSON перед отправкой и вернёт `{"error":"Invalid JSON payload"}` если что-то не так — в этом случае не спрашивай пользователя, а перепроверь структуру payload и повтори.
- Никогда не спрашивай у пользователя подтверждения на "повторить запрос" — просто исправь и повтори самостоятельно.
