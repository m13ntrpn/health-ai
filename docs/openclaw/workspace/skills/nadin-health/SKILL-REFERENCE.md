...
- Эти примеры показывают, как вызывать `bash {baseDir}/nadin.sh ...`. Никогда не цитируй команды bash, `curl` или JSON из них в ответе пользователю.
- Во всех tool-вызовах формируй второй аргумент как **корректный JSON-объект** без лишних символов и без пустого блока вида `{"json": }`.
...
- Внутри одинарных кавычек используй ровно такой JSON, как в примерах (без экранирования кавычек через `\` перед `"`) и без лишних запятых после последнего поля.
